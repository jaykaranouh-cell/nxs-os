...
- Maya (the orchestrator) has write-tools (`src/lib/orchestrator/tools.ts`): create_memory_entry, update_lead_stage, create_agent_task, log_idea, update_opportunity. She also commands her own team mid-conversation: dispatch_agent sends a department agent to investigate, spawn_agent creates a one-off specialist with instructions Maya writes herself (logged as agentId adhoc), message_team leaves notes for departments. Agents communicate with each other: during a run each department agent can ask_agent another department (depth-limited to 1) and send_message notes to teammates/Maya/all via the agent_messages mailbox; unread mail is delivered with the recipient's next briefing and Maya sees the orchestrator/all channel in her context. All comms mirror to agent_logs. Maya names her agents via name_agent (roster persisted in system_context agent-roster, overlaid everywhere: prompts, logs, team channel, /agents); spawn_agent accepts an optional personal name. Jay talks to agents directly via POST /agents/:id/ask — question and reply land in the team channel (from/to 'jay') with an Ask composer on the Agent Layer page. All agents (Maya AND the four departments) have a shared headless browser (`src/lib/orchestrator/browser.ts`, Playwright/Chromium, persistent profile at ~/.nxs-browser-profile): web_search (DuckDuckGo Lite) and browse_page (renders JS, reads logged-in pages). READ-ONLY by design — no click/type/submit, available in amber+green. Search engines often bot-block the headless browser; browse_page on direct URLs is the reliable path. Department-agent tool loop runs WITHOUT extended thinking (thinking + multi-round tool use was unreliable) and always forces a final text answer. In Full Auto (green) Maya additionally gets Mac-control tools: open_in_browser, open_app (execFile, no shell; http(s)-only URLs). The chat synthesis runs a tool-use loop (max 5 rounds); executed actions stream to the UI as `action` SSE events and land in `agent_logs`. The Orchestrator page's execution level gates this: green/amber = tools enabled, red = propose-only (tools withheld, Maya phrases actions for approval)
- Each department agent has its own profile (`src/lib/orchestrator/profile.ts`, `agent_kb` table + `agent-instructions` in system_context): standing **instructions**, **skills** (playbooks), **knowledge** (reference docs), and **private memory** the agent writes itself via a `remember` tool. All four layer onto the agent's persona on top of the shared business briefing. Maya can shape her team with `instruct_agent`/`teach_agent` tools; Jay edits via the Agent Layer → Profile modal (GET /agents/:id/profile, PUT /agents/:id/instructions, POST/DELETE /agents/:id/kb)
...
- Scheduler (`src/lib/scheduler.ts`, node-cron, server-local time): 07:00 morning brief generation, 07:30 risk watchdog (stale critical/needs-review items become high-priority orchestrator tasks), Monday 08:00 one proactive idea per department into the Ideas queue, 17:00 Maya's autonomous growth session (`src/lib/orchestrator/autonomy.ts`: she reviews the business, dispatches/spawns agents as needed, queues 1-3 opportunities/ideas/tasks via tools, and leaves a 🌱 briefing in the chat thread; manual trigger POST /chat/growth-session; 18h min between runs unless forced). LLM jobs no-op without ANTHROPIC_API_KEY
