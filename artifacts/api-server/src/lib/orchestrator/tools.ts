...
const CONTENT_PLATFORMS = ["linkedin", "instagram", "tiktok", "youtube"] as const;
...
const draftPost = {
  definition: {
    name: "draft_post",
    description:
      "Write a social post for any platform (LinkedIn, Instagram, TikTok, YouTube) and queue it in Content Studio for Jay to review and publish. Use when content would help (a win, a lesson, positioning, a launch) or when Jay asks. Write the FULL post ready to publish, tuned to the platform (LinkedIn = professional/insightful; Instagram = punchy + visual; TikTok = hook-driven script; YouTube = title + description/script). Specify which brand/client it's for if relevant. You draft and queue it; Jay does the final publish.",
    input_schema: {
      type: "object" as const,
      properties: {
        content: { type: "string", description: "The full post/caption/script, ready to publish" },
        platform: { type: "string", enum: [...CONTENT_PLATFORMS], description: "Target platform; defaults to linkedin" },
        brand: { type: "string", description: "Brand/client name this is for (e.g. 'NXS AI' or a client). Defaults to own brand." },
  ...
  async run(input: z.infer<typeof draftPostSchema>): Promise<string> {
    const platform = input.platform ?? "linkedin";
    // Resolve brand name → id (match existing, else fall back to own brand).
    let brandId: number | null = null;
    const brands = await db.select().from(brandsTable);
    if (input.brand) {
      const match = brands.find((b) => b.name.toLowerCase() === input.brand!.toLowerCase());
      brandId = match?.id ?? null;
    }
    if (brandId === null) brandId = brands.find((b) => b.isSelf === "true")?.id ?? null;
    ...
    await logAction("Drafted post", `#${row.id} [${platform}]: ${row.hook}`);
    await notifyJay(`${platform} draft ready`, `${row.hook}\n\nReview & publish in Content Studio.`).catch(() => {});
    return `${platform} post drafted and queued in Content Studio (#${row.id}). Open it to add visuals and publish.`;
  },
};

const BASE_TOOLS = [createMemoryEntry, createLead, updateLeadStage, createAgentTask, logIdea, updateOpportunity, createOpportunity, draftPost, dispatchAgent, spawnAgent, messageTeam, nameAgent, notifyJayTool, instructAgent, teachAgent, createObjectiveTool, updateObjectiveTool];
...
- Something worth saying publicly (a win, lesson, launch, sharp take, positioning) → write a post with draft_post for the right platform (LinkedIn, Instagram, TikTok, YouTube) and the right brand/client, and queue it in Content Studio. You write it in voice; Jay keeps the final publish.
- Work that needs investigation or expertise you don't have on hand → dispatch_agent (sales, marketing, research, finance) for analysis, or spawn_agent to stand up a one-off specialist with instructions you write. Use this SPARINGLY — most messages you answer directly and instantly from the context you already have; only involve the team when a task genuinely needs live research or a specialist deep-dive, because every agent you call makes Jay wait. When you do, integrate and credit their work.
